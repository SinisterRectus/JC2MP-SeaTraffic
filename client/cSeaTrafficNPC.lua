...
function SeaTrafficNPC:__init(args)
	...
	self.t = self.vehicle:GetValue("STT")
	self.network_t = self.t
...
function SeaTrafficNPC:Tick(dt)
	...
	self.t = self.t + speed * dt / self.distance
	...
	local p2, q2 = math.bezier(path[1], path[2], path[4] - 2 * (path[4] - path[3]), path[3], self.t)
	
	p2.y = p1.y; q2.roll = q1.roll; q2.pitch = q1.pitch
	
	local dp = p1:Distance(p2)
	local dq = q1:Delta(q2)

	local p = (IsNaN(dp) or dp == 0) and p1 or math.lerp(p1, p2,min((dp + speed) * dt / dp, 1))
	local q = (IsNaN(dq) or dq == 0) and q1 or Angle.Slerp(q1, q2, min(2 * dt / dq, 1))

	vehicle:SetPosition(p)
	vehicle:SetAngle(q)
	
	self.actor:SetInput(Action.Accelerate, min((dp / dt) / self.top_speed, 1))
